...
# Optional heavy dependencies - import lazily to allow SQLite-only usage
try:
...
class SqlEnv:
    ...
    def start_db_sf(self, ex_id):
        if not _SNOWFLAKE_AVAILABLE:
            raise ImportError("snowflake-connector-python is not installed. Run: pip install snowflake-connector-python")
    ...
    def exec_sql_bq(self, sql_query, save_path, max_len):
        if not _BIGQUERY_AVAILABLE:
            raise ImportError("google-cloud-bigquery is not installed. Run: pip install google-cloud-bigquery")
