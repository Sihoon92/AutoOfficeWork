class Prompts:
    ...
    def get_format_prompt(self):
        ...
        format_prompt += "If you meet with an ambiguous name in the task that may match 2 columns, feel free to add 2 columns (like name and name_id) of them. e.g. Tell me the tract code. Tract code may mean geo_id or tract_ce, then format: ```csv\ngeo_id,tract_ce\nid,code``` e.g. Which products were picked for order. Format: ```csv\nproduct_id,product_name,average_units_picked_per_batch\nproduct_id1:int,product_name1:str,average_units1:float\nproduct_id2:int,product_name2:str,average_units2:float\n...```\n"
