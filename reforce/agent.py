...
class REFORCE:
        ...
        while itercount < args.max_iter:
            ...
            if executed_result == 0:
                ...
                if args.save_all_results:
                    save_path = csv_save_path[:-4] + str(itercount) + csv_save_path[-4:]
    ...
    def vote_result(self, search_directory, task, chat_session: GPTChat, sql_paths, table_info):
        ...
        for sql, csv_file in sql_paths.items():
            sql_path = os.path.join(search_directory, sql)
            csv_path = os.path.join(search_directory, csv_file)
            logfile_path = os.path.join(search_directory, csv_file[0] + self.log_save_name)
            try:
